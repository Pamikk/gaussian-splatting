...
ssim = SSIM()
def train(iteration,pipe,background,opt,rank,viewpoint_cam,gaussians,logs,lock,lock_log):
    ...
    with torch.no_grad():
        ...
        if iteration < opt.densify_until_iter:
            ...
            if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                ...
                gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, opt.cameras_extent, size_threshold)
            
            if iteration % opt.opacity_reset_interval == 0 or (opt.white_background and iteration == opt.densify_from_iter):
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    accum = torch.cuda.device_count()
    
    opt.cameras_extent = scene.cameras_extent
    opt.white_background = dataset.white_background
    ...
    loss_cal_time=0
    logs = [ema_loss_for_log,train_time_accum,loss_cal_time_accum,loss_time_accum,cuda_time_accum,densify_time_accum,optimize_time_accum,0,0]
    gaussians.share_memory()
    manager = mp.Manager()
    logs = manager.list(logs)
    ...
    print("\t \t|-- cuda time: ", cuda_time_accum/1000)
