...
#from https://github.com/lartpang/MSSIM.pytorch/blob/main/ssim.py
class GaussianFilter2D(nn.Module):
    ...
    def _get_gaussian_window1d(self):
        ...
        w = w / w.sum()
        return w.reshape(1, 1, self.window_size)
    ...
    def forward(self, x):
        if self.ensemble_kernel:
            ...
            x = F.conv2d(input=x, weight=self.gaussian_window, stride=1, padding=self.padding, groups=x.shape[-3])
        else:
...
class SSIM(nn.Module):
    ...
    def __ssim(self, x, y):
        ...
        mu_xy = mu_x * mu_y
        mu_yy = mu_y * mu_y 
        sigma2_x = self.gaussian_filter(x * x) - mu_xx  # equ 15
        sigma2_y = self.gaussian_filter(y * y) - mu_yy # equ 15
        sigma_xy = self.gaussian_filter(x * y) - mu_xy # equ 16

        A1 = 2 * mu_xy + self.C1
        A2 = 2 * sigma_xy + self.C2
        B1 = mu_xx + mu_yy + self.C1
